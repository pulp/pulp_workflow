...
COMPONENT_SOURCE="./pulp_workflow/dist/pulp_workflow-${COMPONENT_VERSION}-py3-none-any.whl"
...
---
scenario: "${TEST}"
plugin_name: "pulp_workflow"
legacy_component_name: "pulp_workflow"
...
image:
  ...
  webserver_snippet: $(test -f pulp_workflow/app/webserver_snippets/nginx.conf && echo -n true || echo -n false )
extra_files:
  - origin: "pulp_workflow"
    destination: "pulp_workflow"
services:
  - name: "pulp"
    image: "pulp:ci_build"
    volumes:
...
if [ "$TEST" = "gcp" ]; then
  ...
  - name: "ci-gcp"
    ...
    command: " -scheme http"
gcp_test: true
pulp_scenario_settings: {"MEDIA_ROOT": "", "STORAGES": {"default": {"BACKEND": "storages.backends.gcloud.GoogleCloudStorage", "OPTIONS": {"bucket_name": "gcppulp", "custom_endpoint": "http://ci-gcp:4443"}}, "staticfiles": {"BACKEND": "django.contrib.staticfiles.storage.StaticFilesStorage"}}, "domain_enabled": true}
pulp_scenario_env: {}
VARSYAML
fi
